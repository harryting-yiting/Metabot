...
public class TrajectoryPlanner : MonoBehaviour
{
    ...
    // ly change
    [SerializeField]
    string m_RosSubscribeName = "joint_states"; // 话题名可能会写错
    ...
    void Start()
    {
        ...
        Debug.Log("I am coming in start()!!");
    ...
    {
        if (response.planningResult.data == true)
        {
    ...
    {
        Debug.Log("I am coming in Subscriber Callback!!");
    ...
    {
        for (int i = 0; i < jointstate.name.Length - 1; i++)
        {
            if(i < jointstate.name.Length - 2)
            {
            ...
            else
            {
                ...
                // 这个的赋值方式应该是unity自己的习惯，直接对 m_LeftGripper.xDrive.target是赋值不了的，因为是值拷贝
                leftDrive.target = (float)(jointstate.position[i]) * Mathf.Rad2Deg;
                rightDrive.target = -(float)(jointstate.position[i+1]) * Mathf.Rad2Deg;
